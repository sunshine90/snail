...
Document.prototype.set = function (path, val, type, options) {
      ...
      while (i--) {
        ...
        if (null != path[key]
          ...
          if ('real' === pathtype || 'virtual' === pathtype) {
            ...
            throw new Error('Field `' + key + '` is not in schemas.');
          }
  ...
  if ('adhocOrUndefined' == pathType && strict) {
    ...
    if (!mixed) {
      if ('throw' == strict) {
        throw new Error("Field `" + path + "` is not in schemas.");
      }
      return this;
    }
...
/**
 * Assigns/compiles `schemas` into this documents prototype.
 *
 * @param {Schema} schema
 * @api private
 * @method $__setSchema
 ...
 *
 * To apply these options to every document of your schemas by default, set your [schemas](#schema_Schema) `toObject` option to the same argument.
 *
 *     schemas.set('toObject', { virtuals: true })
 *
 * ####Transform
 *
 ...
 * - `options` The options in use (either schemas options or the options passed inline)
 *
 * ####Example
 *
 *     // specify the transform schemas option
 *     if (!schemas.options.toObject) schemas.options.toObject = {};
 *     schemas.options.toObject.transform = function (doc, ret, options) {
 *       // remove the _id of every document before returning the result
 *       delete ret._id;
 *     }
 *
 *     // without the transformation in the schemas
 ...
 *
 * _Note: if you call `toObject` and pass any options, the transform declared in your schemas options will __not__ be applied. To force its application pass `transform: true`_
 *
 *     if (!schemas.options.toObject) schemas.options.toObject = {};
 *     schemas.options.toObject.hide = '_id';
 *     schemas.options.toObject.transform = function (doc, ret, options) {
 *       if (options.hide) {
 ...
 *
 * See [schemas options](/docs/guide.html#toObject) for some more details.
 *
...
Document.prototype.toObject = function (options) {
  ...
  // bypassing the custom schemas options.
  ...
  // child schemas has a transform (this.schemas.options.toObject) In this case,
  // we need to adjust options.transform to be the child schemas's transform and
  // not the parent schemas's
  if (true === transform ||
 ...
 *
 * This method accepts the same options as [Document#toObject](#document_Document-toObject). To apply the options to every document of your schemas by default, set your [schemas](#schema_Schema) `toJSON` option to the same argument.
 *
 *     schemas.set('toJSON', { virtuals: true })
 *
 * See [schemas options](/docs/guide.html#toJSON) for details.
 *
 * @param {Object} options
 * @return {Object}
