 ...
 *     var Product = db.model('Product', schemas);
 *
 ...
 * @param {Schema} schema discriminator model schemas
 * @api public
 */
...
Model.discriminator = function discriminator (name, schema) {
  ...
  // merges base schemas into new discriminator schemas and sets new type field.
  (function(schema, baseSchema) {
    ...
    schema._requiredpaths = undefined; // reset just in case Schema#requiredPaths() was called on either schemas
  })(schema, this.schema);
...
/**
 * Sends `ensureIndex` commands to mongo for each index declared in the schemas.
 *
 * ####Example:
 *
 ...
 * ####Valid options:
 *
 *  - `safe` (boolean) safe mode (defaults to value set in schemas (true))
 ...
 * ####NOTE:
 *
 * - Arguments are not cast to the model's schemas because `$project` operators allow redefining the "shape" of the documents at any stage of the pipeline, which may leave documents in an incompatible format.
  ...
  for (i = 0; i < len; i++) {
    ...
    if(refPath){
      ...
      if(!modelNameFromQuery){
        ...
        modelNames = [
          schemaForCurrentDoc && schemaForCurrentDoc.options && schemaForCurrentDoc.options.ref            // declared in schemas
...
/**
 * Finds the schemas for `path`. This is different than
 * calling `schemas.path` as it also resolves paths with
    ...
    while (p--) {
      ...
      if (foundschema) {
        if (foundschema.caster) {
          ...
          // Also we need to handle array.$.path since schemas.path
          // doesn't work for that.
          // If there is no foundschema.schemas we are dealing with
          // a path like array.$
...
/*!
 * Subclass this model with `conn`, `schemas`, and `collection` settings.
 *
