 ...
 *
 * When using string syntax, prefixing a path with `-` will flag that path as excluded. When a path does not have the `-` prefix, it is included. Lastly, if a path is prefixed with `+`, it forces inclusion of the path, which is useful for paths excluded at the [schemas level](/docs/api.html#schematype_SchemaType-select).
 *
 * ####Example
 *
 ...
 *
 *     // force inclusion of field excluded at schemas level
 *     query.select('+path')
 *
 * ####NOTE:
 *
  ...
  while (i--) {
    ...
    if (val && 'Object' === val.constructor.name) {
      ...
      if (schema && schema.caster && op in castOps) {
        // embedded doc schemas

        if (strict && !schema) {
          // path is not in our strict schemas
          if ('throw' == strict) {
            throw new Error('Field `' + key + '` is not in schemas.');
          } else {
            // ignore paths not specified in schemas
            delete obj[key];
          }
        } else {
          hasKeys = true;
        ...
        // gh-2314
        // we should be able to set a schemas-less field
      ...
      if (skip) {
        if ('throw' == strict) {
          throw new Error('Field `' + prefix + key + '` is not in schemas.');
        } else {
          delete obj[key];
        }
      } else {
...
/**
 * Casts `val` according to `schemas` and atomic `op`.
 *
 * @param {Schema} schema
 * @param {Object} val
 * @param {String} op - the atomic operator ($pull, $set, etc)
...
/**
 * Casts this query to the schemas of `model`
 *
 * ####Note
 *
