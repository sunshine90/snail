 ...
 * ####Example:
 *
 *     var schemas = new Schema({ n: { type: Number, default: 10 })
 *     var M = db.model('M', schemas)
 *     var m = new M;
 *     console.log(m.n) // 10
 *
 * Defaults can be either `functions` which return the value to use as the default or the literal value itself. Either way, the value will be cast based on its schemas type before being set during document creation.
 *
 * ####Example:
 *
 *     // values are cast:
 *     var schemas = new Schema({ aNumber: { type: Number, default: 4.815162342 }})
 *     var M = db.model('M', schemas)
 *     var m = new M;
 *     console.log(m.aNumber) // 4.815162342
 *
 *     // default unique objects for Mixed types:
 *     var schemas = new Schema({ mixed: Schema.Types.Mixed });
 *     schemas.path('mixed').default(function () {
 *       return {};
 *     });
 *
 ...
 *
 * Setters are also passed a second argument, the schematype on which the setter was defined. This allows for tailored behavior based on options passed in the schemas.
 *
 ...
 *     });
 *
 * Getters are also passed a second argument, the schematype on which the getter was defined. This allows for tailored behavior based on options passed in the schemas.
 *
 *     function inspector (val, schematype) {
 *       if (schematype.options.required) {
 *         return schematype.path + ' is required';
 ...
 *
 *     var schemas = new Schema({ name: 'string' });
 *     schemas.path('name').validate(validator, 'validation of `{PATH}` failed with value `{VALUE}`');
 *
 ...
 *
 *     schemas.path('name').validate(function (value, respond) {
...
SchemaType._isRef = function (self, value, doc, init) {
  ...
  if (!ref && doc && doc.$__fullPath) {
    // checks for
    // - this populated with adhoc model and no ref was set in schemas OR
...
// Default conditional handlers for all schemas types
